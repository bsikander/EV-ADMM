...
public class Utils {
	...
	/*
	 * This function loads the aggregator mat file from file system and
	 * populates the MasterData object.
	 */
	public static MasterData LoadMasterDataFromMatFile(String filePath,
		...
		try {
			...
			MasterData context = new MasterData(
					...
					getSingleArrayFromDouble(DArray), priceArray[0]);
	 ...
	 * populates the MasterData object.
	 */
	public static MasterDataValley LoadMasterDataValleyFillingFromMatFile(
		...
		try {
			...
			System.out
					.println("Exception in LoadMasterDataValley function in Utils "
							+ e.getMessage() + " == filePath: " + filePath);
			return null;
		} finally {
			if (tempMatFile != null) {
	 ...
	 */
	private static FileSystem getFSObject(Configuration conf)
	 ...
	 */
	public static File getFileFromHDFS(Configuration conf, String filePath)
			throws IOException, URISyntaxException {
		FileSystem fs = getFSObject(conf);
		FSDataInputStream in = fs.open(new Path(filePath));
		File tempMatFile = stream2file(in,
	...
	public static SlaveData LoadSlaveDataFromMatFile(String filePath,
		...
		try {
			...
			// Text("X_OTIMAL NOT FOUND .. WRITING ZERO"));
			...
			//
			// peer.write(new IntWritable(1), new
			// Text(Utils.convertDoubleArrayToString(getSingleArrayFromDouble(x_optimal))));
			...
			System.out
					.println("Exception in LoadSlaveDataFromMatFile function in Utils -> FILE: "
	...
	/*
	 * This function stores the optimal X value back to mat file. This function
	 * is not used anymore because writing data to takes too much time.
	 */
	public static void SlaveXToMatFile(String filePath, double[] x,
		...
		try {
			...
			final File tempFile = File.createTempFile("saveTemp", ".tmp"); // Create
																			// a
																			// temp
																			// file
			tempFile.deleteOnExit();
			// tempFile.delete()
			String tempPath = tempFile.getAbsolutePath(); // Get absolute path
															// of temp file

			matfileWriter.write(tempPath, list); // Write to temp file

			FileSystem fs = getFSObject(conf);
			fs.copyFromLocalFile(new Path(tempPath), new Path(filePath)); // Copy
																			// the
																			// temp
																			// file
																			// to
			...
			System.out
					.println("Exception in SlaveXToMatFile function in Utils -> FilePath -> "
							+ filePath + " Exception" + e.getMessage());
		} finally {
			if (tempMatFile != null) {
				tempMatFile.delete();
	...
	/*
	 * While loading the EVs, the mat reader returns a double array e.g 96 * 1.
	 * This function converts that array to a single array.
	 */
	...
	/*
	 * This function returns an array of a specified size with all 0's as the
	 * default value.
	 */
	...
	/*
	 * This function returns a double array of a specific size and data
	 * specified by user.
	 */
	...
	/*
	 * This function multiplies all the index of array with a specific value
	 * specified by user.
	 */
	...
	/*
	 * This function replaces the whole column in a double array with an array
	 * array specified by user.
	 */
	public static double[][] setColumnInMatrix(double[][] original,
			double[] column, int columnId) {
	 ...
	 */
	public static double[] calculateMean(double[] xMasterOptimal,
			double[] xSlaveAverageOptimal, int evCount) {
		double[] average = Utils
				.vectorAdd(xMasterOptimal, xSlaveAverageOptimal);
	...
	public static double[][] calculateMatrixSubtraction(double[][] mat1,
			double[][] mat2) {
	...
	public static double[] calculateVectorSubtraction(double[] vec1,
			double[] vec2) {
	...
	public static double[] calculateEVSum(double[][] evData) {
		...
		for (int i = 0; i < evData.length; i++) {
			double sum = 0;
			for (int j = 0; j < evData[0].length - 1; j++) // Donot include
															// master
															// optimization
			{
				sum = sum + evData[i][j];
	...
	public static double calculateNorm(double[] vec) {
		...
		// System.out.println(">>>> Norm: " + v.getNorm() + " -- L1 Norm: " +
		// v.getL1Norm() + " -- Inf Norm: " + v.getLInfNorm());
		// return v.getL1Norm();
		return v.getNorm();
	...
	public static String networkMasterToJson(NetworkObjectMaster context)
			throws IOException {
		return OBJECT_MAPPER.writeValueAsString(context);
	}

	public static String networkSlaveToJson(NetworkObjectSlave context)
			throws IOException {
		return OBJECT_MAPPER.writeValueAsString(context);
	}

	public static NetworkObjectMaster jsonToNetworkMaster(String json)
			throws IOException {
		return OBJECT_MAPPER.readValue(json, NetworkObjectMaster.class);
	}

	public static NetworkObjectSlave jsonToNetworkSlave(String json)
			throws IOException {
