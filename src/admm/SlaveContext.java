...
public class SlaveContext {
	 ...
	 */
	public SlaveContext(SlaveData slaveData, double[] xMean, double[] u,
			int currentEVNo, double rhoValue, boolean isFirstIteration,
			BSPPeer<LongWritable, Text, IntWritable, Text, Text> peer,
			double delta, double[] oldXOptimal) throws IOException {
	...
	/*
	 * This function solves the model using IBM CPLEX and returns the cost. The
	 * key for a successful long run is that you do not create the CPLEX
	 * instances again and again because the objects are not cleaned up by GC.
	 * So, always use clearModel to clear out the old model and create a new one
	 * and reuse the same cplex object again and again.
	 */
	public double optimize(IloCplex cplex) throws IloException,
		...
		for (int i = 0; i < this.x.length; i++) {
			...
			exps[i] = cplex.prod(rho / 2,
					cplex.square(cplex.sum(x_i[i], cplex.constant(data[i]))));
		...
		// {
		// //exps[i] = cplex.sum(cplex.prod(gammaAlpha, cplex.square(x_i[i]))
		// ,cplex.prod(rho/2, cplex.square(cplex.sum(x_i[i],
		// cplex.constant(data[i])))));
		// exps[i] = cplex.prod(rho/2, cplex.square(cplex.sum(x_i[i],
		// cplex.constant(data[i]))));
		//
		// AXExpEq[i] = cplex.prod(x_i[i], this.slaveData.getA()[i]);
		// }
		...
		// {
		// //cplex.addEq(cplex.prod(x_i[j], this.slaveData.getA()[j]),
		// this.slaveData.getR());
		...
		// {
		// IloNumExpr[] BXExpLe = new
		// IloNumExpr[this.slaveData.getB()[0].length];
		// IloNumExpr[] BXExpGe = new
		// IloNumExpr[this.slaveData.getB()[0].length];
		//
		//
		...
		if (cplex.solve()) {
			...
			xOptimalDifference = Utils.calculateVectorSubtraction(x_optimal,
					this.x);
			...
			System.out.println("RHO -> " + this.rho + " >> Alpha->"
					+ this.alpha + " >>gamma ->" + this.gamma);
	...
	/*
	 * Getter to access the difference of old optimal value and current optimal
	 * value (x_old* - x*)
	 */
