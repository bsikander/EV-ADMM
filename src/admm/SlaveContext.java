...
public class SlaveContext {
	...
	public SlaveContext(String fileName, double[] xMean, double[] u, double[] x, int currentEVNo)
	{	
	...
	public double optimize() throws IloException
	{
		...
		//IloNumExpr[] AXExpGe = new IloNumExpr[data.length];
...
//				cplex.addLe(cplex.prod(x_i[f],this.slaveData.getB()[h][f]), this.slaveData.getSmax()[h]);
//				cplex.addGe(cplex.prod(x_i[f],this.slaveData.getB()[h][f]), this.slaveData.getSmin()[h]);
			}
