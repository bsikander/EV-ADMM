...
public class MasterContextValley {
	...
	/*
	 * After loading the Aggregator data, everything is populated in this
	 * object.
	 */
	...
	/*
	 * This is the default constructor of this class and is responsible to load
	 * the Aggregator file specified by user through command line parameters and
	 * to scale the value of D based on the total EVs.
	 */
	public MasterContextValley(String inputPath, int evCount, double rhoValue,
			Configuration conf) {
		masterData = Utils.LoadMasterDataValleyFillingFromMatFile(inputPath,
				conf);
		N_EV = evCount;
		rho = rhoValue;
	...
	/*
	 * This function solves the Aggregator model, set the optimal value in class
	 * variable and returns the cost.
	 */
	public double optimize(double[] xold, int iteration) throws IloException,
		...
		// this.setXOptimal( Utils.calculateVectorSubtraction(rhoMultiplyK,
		// rhoMultiplyD ));
		this.setXOptimal(Utils.vectorAdd(rhoMultiplyK, rhoMultiplyD));

		// cost= norm(D-x)^2;
		double cost = Utils.calculateNorm(Utils.vectorAdd(masterData.getD(),
				Utils.scalerMultiply(this.getXOptimal(), -1)));

		return cost * cost;
	}

	/*
	 * This function scales the Demand parameter of Aggregator based on the
	 * total EVs.
	 */
	...
	private double[] subtractOldMeanU(double[] xold) {
		double[] temp = Utils.vectorAdd(xold,
				Utils.scalerMultiply(this.xMean, -1));
		double[] output = Utils.vectorAdd(temp,
				Utils.scalerMultiply(this.u, -1));
	...
	/*
	 * Returns the masterData object which is created after loading the
	 * Aggregator mat file.
	 */
